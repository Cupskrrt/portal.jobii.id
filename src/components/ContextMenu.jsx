...
const ContextMenu = ({ style, onOptionClick, path, itemName }) => {
  ...
  const handleRenameItem = async (itemName) => {
    const newName = window.prompt('Enter the new name', itemName.split('.')[0]);
    if (newName) {
      const newNameWithExtension = `${newName}.${itemName.split('.').pop()}`;
      await renameItem({ itemName, newName: newNameWithExtension });
      console.log(`${itemName} renamed to ${newNameWithExtension}`);
    }
  };

  const directory = path.replace(itemName, '');
  

  return (
    <div className="fixed bg-white border rounded shadow-md" style={style}>
      <div className="py-1">
        <a href={`http://localhost:5000/download/${directory}${itemName}`} className="flex items-center block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => onOptionClick('download')}>
